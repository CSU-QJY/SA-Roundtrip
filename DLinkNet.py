...
from tensorflow.python.keras.layers import Conv2D, MaxPooling2D, Input, Conv2DTranspose,AveragePooling2D, \
    Add, Activation, \
    BatchNormalization
...
def encoder_block(input_tensor, num_filters, num_res_blocks):
    ...
    while num_res_blocks > 1:
        ...
        num_res_blocks -= 1
    encoded_pool = Conv2D(num_filters,3,2,padding='same')(encoded)
    encoded_pool = BatchNormalization()(encoded_pool)
    encoded_pool = Activation('relu')(encoded_pool)
    return encoded, encoded_pool


def create_dlinknet_en(input_shape):
    inpu=tf.keras.layers.Input(shape=input_shape)
    inputs_ = Conv2D(64, kernel_size=(3, 3), padding='same')(inpu)
    inputs_ = BatchNormalization()(inputs_)
    inputs_ = Activation('relu')(inputs_)
    max_pool_inputs = Conv2D(64,3,2,padding='same')(inputs_)
    max_pool_inputs = BatchNormalization()(max_pool_inputs)
    max_pool_inputs = Activation('relu')(max_pool_inputs)
    ...
    # output=tf.keras.layers.Flatten()(center)
    # output=tf.keras.layers.Dense(output.shape[1])(output)
    ...
    outputs = Conv2D(1, (1, 1))(final)


    return tf.keras.Model(inputs=inpu,outputs=outputs)


if __name__ == '__main__':
    c=create_dlinknet_en(input_shape=[128,128,1])
    c.summary()