...
imgs = glob.glob(os.path.join('preprocessedData', 'lesion_type_5', '*', '*.png'))
...
def imread(f, mode='gan'):
    x = cv2.imread(f)
    if mode == 'gan':
        x = cv2.resize(x, (img_dim, img_dim))
...
g_train_model.metrics.append(K.mean(xz_real_score - xz_fake_score))
g_train_model.metrics_names.append('r_loss')
g_train_model.metrics.append(2 * K.mean(K.square(z_fake - z_fake_)) + 3 * K.mean(K.square(x_real - x_real_)))
...
def sample(path, n=8, z_samples=None):
    ...
    figure=rescale_intensity(figure,out_range=(0,255)).astype(np.uint8)
